...
class SystemAdminServices extends BaseServices
{
    ...
    public function batchRole($param,$adminInfo){
        $admin_ids_arr = explode(',',$param['admin_ids']);
        $role_ids_arr =  array_filter( explode(',',$param['role_ids']) );
        if( count($role_ids_arr) > 1){
            return ['status'=>0,'msg'=>'目前只支持单个角色'];
        }
        $role_ids_arr = array_values($role_ids_arr);
        $role_id = $role_ids_arr[0];
        if($adminInfo['role_id'] > $role_id){
            return ['status'=>0,'msg'=>'不能设置高于自己角色权限的角色'];
        }
        try {
            foreach($admin_ids_arr as $key => $value){
                // todo  权限判断
                $this->dao->update($value,['role_id'=>$role_id,'roles'=>$role_id]);
            }
    ...
    public function batchStatusService($param)
    {
        try {
            $this->dao->update([['id', 'in', $param['ids']]], ['status'=> $param['status']]);
            return ['status'=>1, 'msg'=>'成功'];
        } catch (\Exception $e){
            return ['status'=>0, 'msg'=>$e->getMessage()];
        }
