...
if (!function_exists('show')) {
    function show($status, $message, $data = null, $httpCode = 200)
    {
        $data =  [
            'code' => $status,
            'msg' => $message,
    ...
    function get_tree_children_value(array $data, $value, string $childrenname = 'children', string $keyName = 'id', array &$childrenValue = [])
    {
        foreach ($data as $item) {
            ...
            if (count($childrenData)) {
                return get_tree_children_value($childrenData, $value, $childrenname, $keyName, $childrenValue);
            } else {
...
if(!function_exists('randomCode')) {
    ...
    */
    ...
    {
        $bag = '';
        $codeSet = '123456789abcdefghijklmnopqrstuvwxyz';
...
if (!function_exists('build_short_url')) {
     ...
     * @return string
     */
    function build_short_url($url)
    {
        $curl = new Curl();
        $data = [
            'key' => '600919466aee8a1df7a6ddf3@5ad8bac97d1627a9ace0451cfa7c2908',
            'url' => $url,
            'format' => 'json',
        ];
        $curl->get('http://api.suolink.cn/api.htm', $data);
        if($curl->error) {
            return '';
...
if (!function_exists('test_log')) {
    function test_log($text) {
        ...
        if(is_array($text)){
            \think\facade\Db::name('test')->save(['info'=>json_encode($text),'create_datetime'=>Date('Y-m-d H:i:s')]);
        }else{
            \think\facade\Db::name('test')->save(['info'=>$text,'create_datetime'=>Date('Y-m-d H:i:s')]);
        }
    }
}
if (!function_exists('test_tmp_log')) {
    function test_tmp_log($text,$method='',$type='') {
        if(Config::get('app.app_host') == 'dev') { 
            test_log($text);
            return; //测试环境
        }
        $save = [];
        $save['create_datetime'] = Date('Y-m-d H:i:s');
        if(is_array($text)){
            $save['info'] = json_encode($text);
        }else{
            if(is_string($text)){
                $save['info'] = $text;
            }else{
        ...
        $key = md5(Date('Hi').$method.$type);
        $save['key'] = $key;
        try{
            app("swoole.table.tmp_log")->set($key,$save);
        }catch(\Exception $e) {
            return;
        }
    }
...
if (!function_exists('system_error_log')) {
    function system_error_log($function_name,$name,$content) {
        $ymdh = Date('ymdH');
        $hasCache = Cache::get('system_error_log_'.$name.$ymdh);
        if(!$hasCache){
            ...
            if(!$has){   
                ...
                Cache::set('system_error_log_'.$name.$ymdh,1);
            }
        }
    }
}

if (!function_exists('getRiskDistrictListCommon')) {
    function getRiskDistrictListCommon(){
        $list = Cache::get('RiskDistrictList');
        if($list) {
            return $list;
        }
        $data = Db::name('risk_district')->field('id,province,city,county,start_date')->whereNull('delete_time')->select()->toArray();
        Cache::set('RiskareaList', $data); //永久保存
        return $data;
    }
}
