...
class Queue
{
    ...
    public function push(?array $data = null)
    {
        ...
        // var_dump('start QueueThink');
        // var_dump('QueueThink::'.$this->action());
        ...
        if (!$res) {
            $res = QueueThink::{$this->action()}(...$jodValue);
            if (!$res) {
                Log::error('加入队列失败，参数：' . json_encode($jodValue));
            }
        }
        $this->clean();
        return $res;
    ...
    protected function getValues($data)
    {
        ...
        // var_dump('getValues '.$this->do);
        // var_dump('defaultDo '.$this->defaultDo);
